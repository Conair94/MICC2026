...
def fix_matrix_signs(M):
    ...
    while 0 in M[1, :, 1:4:2] and row < M.shape[1]-1:
        ...
        for i in range(M.shape[1]):
            ...
            ind = int(new[np.where(new % 2 == col % 2)][0])
            sgn = -sgn
            M[1, g_val, ind] = sgn
            ind += 2
            ind = ind % M.shape[2]
...
def build_matrices(edge_paths, all_paths):
    ...
    for path in all_paths:
        ...
        mapped_paths.append(dict(zip(ordered_path,list(range(len(path))))))
    #print mapped_paths,'\n'
    ...
    for Path, mapped_path in zip(all_paths, mapped_paths):
        ...
        M = np.zeros(shape)
        M2 = np.zeros(shape)
        M[:, 0] = np.array([path_size-1]+list(range(path_size-1)))
        M[:, 2] = np.array(list(range(1,path_size))+[0])
        past_edges = dict()
        future_edges = dict()
        old_vertex = last
...
def find_combined_paths(all_paths, M_library):
    ...
    '''
    list_of_connected = []
    path_library = dict(zip(list(range(len(all_paths))), all_paths))
    index1 = 0
...
def edges(M):
    ...
    facesTemp = dict()
    for i,j in product(list(range(num_rows)),list(range(num_cols))):
        ...
        while not found:
            ...
            j_next = (int(new[ind][0])+1)%num_cols #Always move clockwise
            j_old = j
            j = j_next
...
def boundary_count(M):
    ...
    for i,j in product(list(range(numrows)),list(range(numcols))):
...
def vector_solution(edges):
    ...
    for face in edges:
        if face[0] not in list(solution.keys()):
            solution[face[0]] = 1
        else:
            solution[face[0]] += 1
...
def genus(M, euler=0, boundaries = 0):
    ...
    #if bigon == 1: P -= 1 # pull away one bigon
...
class CurvePair:
    ...
    def __init__(self, top_beta, bottom_beta, dist=1, conjectured_dist=3,recursive=False):
        ...
        self.matrix[0,:,0] = [self.n-1] + list(range(self.n-1))
        self.matrix[0,:,1] = self.top
        self.matrix[0,:,2] = list(range(1,self.n)) +[0]
    ...
    def compute_distance(self, M, all_paths,recursive=True):
        ...
        if dist == 3:
            ...
            if recursive:
                geodesic_distances = []
                for k, matrix in six.iteritems(lib):
                ...
                #print '\n'
                return int(min(set(geodesic_distances))) + 1, lib
            else:
                return dist, lib
